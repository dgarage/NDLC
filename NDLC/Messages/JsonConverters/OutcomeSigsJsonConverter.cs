...
namespace NDLC.Messages.JsonConverters
{
    ...
    class OutcomeSigs2JsonConverter : JsonConverter<Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature>>
    {
        public override Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature> ReadJson(JsonReader reader, Type objectType, [AllowNull] Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature> existingValue, bool hasExistingValue, JsonSerializer serializer)
        {
            if (reader.TokenType is JsonToken.Null)
                return null!;
            Expect(reader, JsonToken.StartArray);
            var result = new Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature>();
            reader.Read();
            while (reader.TokenType != JsonToken.EndArray)
            {
                Expect(reader, JsonToken.StartObject);
                ...
                if (!TryParse((string)reader.Value!, out var sig) || sig is null)
                    throw new JsonObjectException("Unexpected token while parsing outcome sigs", reader);
                if (!result.TryAdd(h, sig))
                    throw new JsonObjectException("duplicate outcome sig", reader);
                reader.Read();
        ...
        private bool TryParse(string str, out SecpECDSAAdaptorSignature? sig)
        {
            try
            {
                var bytes = Encoders.Hex.DecodeData(str);
                return SecpECDSAAdaptorSignature.TryCreate(bytes, out sig);
            }
            catch
            {
                sig = null;
        ...
        public override void WriteJson(JsonWriter writer, [AllowNull] Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature> value, JsonSerializer serializer)
        {
            if (value is Dictionary<DiscreteOutcome, SecpECDSAAdaptorSignature>)
            {
                writer.WriteStartArray();
                foreach (var kv in value)
                {
                    writer.WriteStartObject();
                    writer.WritePropertyName(Encoders.Hex.EncodeData(kv.Key.Hash));
                    writer.WriteValue(Encoders.Hex.EncodeData(kv.Value.ToBytes()));
                    writer.WriteEndObject();
                }
                writer.WriteEndArray();
            }
